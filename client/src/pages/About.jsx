...
const VISION_META = [
  { key: 'vision', label: 'Our Vision', icon: FiEye, bg: 'linear-gradient(135deg, var(--forest-green), var(--mid-green))' },
  { key: 'mission', label: 'Our Mission', icon: FiCompass, bg: 'linear-gradient(135deg, var(--navy-dark), var(--navy))' },
  { key: 'tagline', label: 'Our Tagline', icon: FiHeart, bg: 'linear-gradient(135deg, var(--earth), var(--earth-light))' },
];

export default function About() {
  const { content } = useSiteContent();
  const { org, vision, values, aboutFacts } = content;
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '4rem', alignItems: 'center' }}>
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
              ...
              <p style={{ color: 'var(--gray-500)', lineHeight: 1.9, fontSize: '1rem' }}>
                Founded in {org.location}, {org.shortName} is a Christ centered ministry serving prisoners,
                former prisoners, single mothers, vulnerable children, unemployed youth, and underserved
                communities through holistic outreach, discipleship, and sustainable development.
              </p>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
      ...
      <section id="vision" style={{ padding: '6rem 0', background: 'var(--cream)' }}>
        <div className="container">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
            {VISION_META.map((item, i) => {
              const Icon = item.icon;
              return (
                <motion.div key={item.key}
                  initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                  transition={{ delay: i * 0.15, duration: 0.6 }}
                  style={{ background: item.bg, borderRadius: '20px', padding: '2.5rem', color: 'white', textAlign: 'center', boxShadow: 'var(--shadow-lg)' }}
                >
                  <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: 'rgba(255,255,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.25rem' }}>
                  ...
                  <p style={{ color: 'rgba(255,255,255,0.85)', lineHeight: 1.8, fontSize: '1rem' }}>{vision[item.key]}</p>
                </motion.div>
              );
            })}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.25rem' }}>
            {(values || []).map((value, i) => (
              <motion.div key={value.name}
                ...
                style={{ padding: '1.5rem', borderRadius: '12px', background: 'var(--gray-50)', border: '1px solid var(--gray-100)', borderLeft: `4px solid ${value.color}` }}
      ...
      <section style={{ padding: '5rem 0', background: 'linear-gradient(135deg, var(--dark-green), var(--navy-dark))', textAlign: 'center' }}>
        <div className="container">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 style={{ fontFamily: 'var(--font-serif)', color: 'white', fontSize: 'clamp(1.8rem, 4vw, 2.75rem)', fontWeight: 700, marginBottom: '1rem' }}>Partner in This Mission</h2>
            <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: '1.05rem', maxWidth: '500px', margin: '0 auto 2rem', lineHeight: 1.8 }}>
              Together we can reach more lives across Uganda.
            </p>
